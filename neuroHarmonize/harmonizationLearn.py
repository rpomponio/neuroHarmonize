...
def harmonizationLearn(
    ...
    model = None
    isTrainSite = None
    isTrainSiteColumns = None
    isTrainSiteColumnsOrig = None
    if orig_model is not None:
        model = orig_model.copy()

    if model is not None:
        # take parameters from original model
        isTrainSite = covars["SITE"].isin(model["SITE_labels"])
        isTrainSiteLabel = set(model["SITE_labels"])
        isTrainSiteColumns = np.where(
            (
                pd.DataFrame(np.unique(covars["SITE"]))
                .isin(model["SITE_labels"])
                .values
            ).flat
        )
        isTrainSiteColumnsOrig = np.where(
            (
                pd.DataFrame(model["SITE_labels"])
                .isin(np.unique(covars["SITE"]))
                .values
            ).flat
        )
        isTestSiteColumns = np.where(
            (
                ~pd.DataFrame(np.unique(covars["SITE"]))
                .isin(model["SITE_labels"])
                .values
    ...
    if smooth_model["perform_smoothing"]:
        ...
        X_spline = covars[:, smooth_cols].astype(float)  # type: ignore
        if model is None:
            bs = BSplines(
                X_spline,
    ...
    if model is None:
        (
            ...
            B_hat,
            grand_mean,
        ) = standardizeAcrossFeatures(data, design, info_dict, smooth_model)
        ...
        # Create train data
        (batch_levels, sample_per_batch) = np.unique(
            covars[isTrainSite, batch_col], return_counts=True
        )
        if batch_levels.size == 0:
            ...
            (
                ...
                stand_mean_train,
                _,
            ) = applyStandardizationAcrossFeatures(
                data[:, isTrainSite],
                tmp[isTrainSite, :],
                info_dict_train,
                model,
            ...
            design2 = tmp.copy()
            design2[:, isTrainSiteColumnsOrig[0]] = design[  # type: ignore
                :, isTrainSiteColumns[0]  # type: ignore
            ]
        ...
        (batch_levels, sample_per_batch) = np.unique(
            covars[~isTrainSite, batch_col], return_counts=True  # type: ignore
        )
        if batch_levels.size == 0:
            ...
            info_dict_test = {
                ...
                "n_batch": len(batch_levels),
                "n_sample":
                    int(covars[~isTrainSite, :].shape[0]),  # type: ignore
                "sample_per_batch": sample_per_batch.astype("int"),
                "batch_info": [
                    list(np.where(
                        covars[~isTrainSite,  # type: ignore
                               batch_col] == idx)[0])
                    for idx in batch_levels
                ],
            }
            design_tmp = np.concatenate(
                (
                    design[:, isTestSiteColumns[0]],  # type: ignore
                    design[:, len(batch_labels):],
                ),
                axis=1,
            )
            (
                ...
                data[:, ~isTrainSite],  # type: ignore
                design_tmp[~isTrainSite, :],  # type: ignore
                info_dict_test,
                model,
            )
            LS_dict = fitLSModelAndFindPriors(
                s_data_test, design_tmp[~isTrainSite, :],  # type: ignore
                info_dict_test, eb=eb
            )
            if eb:
                gamma_star, delta_star = find_parametric_adjustments(
            ...
            for i in range(info_dict_test["n_batch"]):
                diff_mean = np.mean(
                    data[:, info_dict_test["batch_info"][i]]
                    - np.dot(
                        design[
                            info_dict_test["batch_info"][i],
                            info_dict["n_batch"]:,
                        ],
            ...
            model["SITE_labels"] = np.append(
                model["SITE_labels"],
                list(set(batch_labels) - isTrainSiteLabel),  # type: ignore
            )
            model["gamma_star"] = np.append(
                model["gamma_star"], gamma_star, axis=0
            )
            model["delta_star"] = np.append(
                model["delta_star"], delta_star, axis=0
            )
            model["info_dict"]["n_batch"] = len(model["SITE_labels"])
            bayes_data_test = adjust_data_final(
                s_data_test,
                design_tmp[~isTrainSite, :],  # type: ignore
                gamma_star,
                delta_star,
        ...
        bayes_data[~isTrainSite, :] = bayes_data_test  # type: ignore
        s_data = np.zeros(shape=data.T.shape)
        s_data[isTrainSite, :] = s_data_train.T
        s_data[~isTrainSite, :] = s_data_test.T  # type: ignore
...
def standardizeAcrossFeatures(X, design, info_dict, smooth_model):
    ...
    if smooth_model["perform_smoothing"]:
        ...
        if X.shape[0] > 10:
            print(
                "\n[neuroHarmonize]: smoothing more than 10 variables "
                "may take several minutes of computation."
            )
        ...
        for i in range(0, X.shape[0]):
            ...
            gam_bs = GLMGam.from_formula(
                formula, data=df_gam, smoother=bs, alpha=alpha
            )
            _ = gam_bs.fit()
            # Optimal penalization weights alpha can be obtained through
            # gcv/kfold
            # Note: kfold is faster, gcv is more robust
            gam_bs.alpha = gam_bs.select_penweight_kfold()[0]  # type: ignore
...
def fitLSModelAndFindPriors(s_data, design, info_dict, eb=True):
    ...
    gamma_hat = np.array(
        np.dot(
            np.dot(
                np.linalg.inv(np.matrix(np.dot(batch_design.T, batch_design))),
                batch_design.T,
            ),
            s_data.T,
        )
...
def saveHarmonizationModel(model, file_name):
    ...
    if os.path.exists(file_name):
        raise ValueError(
            "Model file already exists: %s. Change name or delete to save."
            % file_name
        )
    ...
    print(
        "\n[neuroHarmonize]: Saving model object, estimated size in MB: %4.2f"
        % est_size
    )
