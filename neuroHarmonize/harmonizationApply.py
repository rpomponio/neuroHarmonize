...
def harmonizationApply(data, covars, model, return_stand_mean=False):
    ...
    site_dict = dict(
        zip(model["SITE_labels"], np.arange(len(model["SITE_labels"])))
    )
    covars[:, batch_col] = np.vectorize(site_dict.get)(
        covars[:, batch_col], -1
    )

    # compute samples_per_batch for training data
    sample_per_batch = [
        np.sum(covars[:, batch_col] == i) for i in list(site_dict.values())
    ]
    ...
    design = make_design_matrix(
        ...
        cat_cols,
        num_cols,
        nb_class=len(model["SITE_labels"]),
    )
    ...
    ###
    s_data, stand_mean, var_pooled = applyStandardizationAcrossFeatures(
        data, design, info_dict, model
    )
...
def applyStandardizationAcrossFeatures(X, design, info_dict, model):
    ...
    tmp = np.concatenate(
        (
            np.zeros(shape=(n_sample, len(model["SITE_labels"]))),
            tmp[:, n_batch:],
        ),
        axis=1,
    )
...
def applyModelOne(data, covars, model, return_stand_mean=False):
    ...
    if batch_i not in batch_labels:
        # raise ValueError(
        #     'Site Label "%s" not in the training set. '
        #     'Check `covars` argument.' % batch_i)
    ...
    design_i = np.concatenate(
        (design_i_batch, design_i[:, len(batch_labels):]), axis=1
    )
    ...
    if sum(isTrainSite) == 0:
        ...
        bayesdata = (
            bayesdata * np.dot(vpsq, np.ones((1, n_sample))) + stand_mean
        )
...
def loadHarmonizationModel(file_name):
    ...
    if not os.path.exists(file_name):
        raise ValueError(
            ...
            "Did you run `saveHarmonizationModel`?"
            % file_name
        )
